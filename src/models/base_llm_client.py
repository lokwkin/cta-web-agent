...
class BaseLLMClient():
    ...
    def prompt_templated(self, template: str, params: dict):
        ...
        [system_message, user_message] = pystache.render(content, params).split(
            '>>>>>>>>>>>>>>>> PROMPT_SEPERATOR >>>>>>>>>>>>>>>>')
        result = self.prompt(LLMInput(user_message=user_message,
                             system_message=system_message, log_identifier=template))
        return result
    ...
    def prompt(self, prompt_input: LLMInput) -> ReActOutput:
        try:
            log_filename = str(int(time.time())) + \
                (f"_{prompt_input.log_identifier}" if prompt_input.log_identifier is not None else '') + ".log"
